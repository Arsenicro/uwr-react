...
interface Props<Value, Multiple extends boolean | undefined = false> extends Pick<Combobox.Root.Props<Value, Multiple>, 'onOpenChange' | 'open' | 'defaultOpen' | 'onValueChange' | 'value' | 'defaultValue'> {
  label: string;
  noItemsFoundText?: string;
  items: { label: string; value: string }[];
}

export default function BaseUICombobox<Value, Multiple extends boolean | undefined = false>({ label, noItemsFoundText, items, ...props }: Props<Value, Multiple>) {
  const id = React.useId();
  return (
    <Combobox.Root items={items} {...props}>
      <div className="relative flex flex-col gap-1 text-sm leading-5 font-bold text-gray-900 dark:text-gray-100">
        ...
        <Combobox.InputGroup className="relative box-content h-10 w-64 rounded-md border border-gray-200 bg-[canvas] focus-within:outline-2 focus-within:-outline-offset-1 focus-within:outline-blue-800 [&>input]:pr-[calc(0.5rem+1.5rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+1.5rem*2)] ">
          <Combobox.Input
            ...
            id={id}
            className="h-full w-full border-0 bg-transparent pl-3.5 text-base font-normal text-gray-900 outline-none dark:text-gray-100 dark:bg-gray-800"
          />
      ...
      <Combobox.Portal>
        <Combobox.Positioner className="outline-none" sideOffset={4}>
          <Combobox.Popup className="w-[var(--anchor-width)] max-h-[23rem] max-w-[var(--available-width)] origin-[var(--transform-origin)] rounded-md bg-[canvas] text-gray-900 shadow-lg shadow-gray-200 outline-1 outline-gray-200 transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0 dark:shadow-none dark:-outline-offset-1 dark:outline-gray-300 duration-100">
            <Combobox.Empty>
              <div className="p-4 text-[0.925rem] leading-4 text-gray-600">{noItemsFoundText || 'No items found.'}</div>
            </Combobox.Empty>
